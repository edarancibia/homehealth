...
public interface PacienteRepository extends JpaRepository<Paciente, Long> {
    ...
    @Query(value = "select f2.idficha, UPPER(CONCAT(p.a_pat,' ',p.a_mat,' ',p.nombre)) as nombre,TIMESTAMPDIFF(YEAR,p.fecha_nac ,CURDATE()) AS edad,\n"
    		+ "	p.rut_num ,f2.fecha \n"
    ...
    @Query(value = "select f2.idficha ,p.rut_num, UPPER(CONCAT(p.a_pat,' ',p.a_mat,' ',p.nombre)) as nombre,TIMESTAMPDIFF(YEAR,p.fecha_nac ,CURDATE()) AS edad \n"
    		+ "    		from paciente p, ficha f2 \n"
